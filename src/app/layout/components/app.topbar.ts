...
<!--                                                <span-->
<!--                                                  class="font-medium text-surface-950 dark:text-surface-0">{{ item.namea }}</span>-->
<!--                        <div-->
            ...
            >
              <ul class="flex flex-col gap-1">
                <li>
                  ...
                  {{ auth?.user?.staff?.prefixnameacad || '' }}{{ auth?.user?.name || '' }}
                </span>
                  </a>
                </li>
<!--                <li>-->
...
export class AppTopbar {
  ...
  auth: any = null; // Initialize as null to prevent errors if not set
  ...
  ngOnInit() {
    ...
    // Subscribe to isAuthenticated status changes from AuthService
    this.authService.isAuthenticated.subscribe(status => {
      this.isAuthenticated = status;
      //alert(status)
      // When authentication status changes, update the 'auth' data
      if (status) {
        this.auth = this.authLocalStorageService.getLocalStorage.auth;
          //console.log('User data updated from AuthLocalStorageService:', this.auth);
      } else {
        this.auth = null; // Clear auth data if not authenticated
        //     console.log('User is no longer authenticated, auth data cleared.');
      }
    });
  }
